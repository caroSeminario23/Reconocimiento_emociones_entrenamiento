...
csv_file_path = 'Procesamiento_datos/Datos/Resultados_temp.csv'

# Abrir el archivo CSV en modo escritura para sobreescribirlo
with open(csv_file_path, 'w', newline='') as csvfile:
    writer = csv.writer(csvfile)
    writer.writerow(['id_res_temp', 'valor_prediccion', 'fecha', 'hora']) 
...
id_res_temp = 0  # Variable para el ID del resultado
...
while cap.isOpened():
    ...
    if results.multi_face_landmarks:
        for face_landmarks in results.multi_face_landmarks:
            ...
            valor_prediccion = format(pred_value, '.3f') # Redondear el valor de predicción con 3 decimales

            write_to_csv(csv_file_path, [id_res_temp, valor_prediccion, fecha, hora])
            id_res_temp += 1
